LANGUAGE_NAME := tree-sitter-whim
HOMEPAGE_URL := https://github.com/carthage-software/tree-sitter-whim
VERSION := 0.1.0
DESCRIPTION := Tree-sitter grammar for Whim
...
install: all
	install -d '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/whim '$(DESTDIR)$(INCLUDEDIR)'/tree_sitter '$(DESTDIR)$(PCLIBDIR)' '$(DESTDIR)$(LIBDIR)'
...
ifneq ($(wildcard queries/*.scm),)
	install -m644 queries/*.scm '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/whim
endif
...
uninstall:
	...
	$(RM) -r '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/whim
